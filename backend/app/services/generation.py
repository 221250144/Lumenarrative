...
GENERATION_DURATION_MARGIN_S = 0.5


def reserved_generation_duration(duration_s):
    # The returned video may include a small tail beyond the requested integer
    # duration (a real 5s request returned 5.125s). Reserve it before billing.
    return duration_s + GENERATION_DURATION_MARGIN_S
...
def check_capacity(db, project_id, duration_s, exclude_job_id=None):
    ...
    if sum(a.duration_s for a in assets) + sum(reserved_generation_duration(j.data.get("duration_s", 0)) for j in pending) + duration_s > settings.max_project_duration_s:
...
def generation_context(db, task_id):
    ...
    for evidence_id in task.data.get("reference_evidence_ids", []):
        ...
        frame = {
            ...
            "caption": str(data.get("action", "补全建议的参考画面")),
        }
...
def readable_task_references(db, task):
    plan = db.get(CompletionPlan, task.plan_id)
    run = db.get(AnalysisRun, plan.analysis_id) if plan else None
    if not run or plan.project_id != task.project_id or run.project_id != task.project_id:
        return ReadableReferences()
    evidence = [
        {**row.data, "id": row.id}
        for row in db.scalars(select(Evidence).where(
            Evidence.project_id == task.project_id, Evidence.analysis_id == run.id,
        ))
    ]
    return ReadableReferences(run.data.get("shots", []), evidence)


def default_prompt(task, references=None):
    data = task.data
    ...
    # Convert full references before applying the public text limit; truncating
    # first could leave an unrecognizable UUID fragment in the editable prompt.
    return (references or ReadableReferences()).text("\n".join(parts))[:2000]
