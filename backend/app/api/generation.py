...
from app.services.generation import generation_context, generation_jobs, default_prompt, retryable, check_capacity
...
def generation_options(id: str, db: DB):
    ...
    result = {
        ...
        "prompt": default_prompt(task), "duration_s": max(3, min(15, duration)),
    ...
    try:
        ...
        result["reference_frames"] = references
        if reason := blocked_previous(jobs):
            raise GenerationError(reason)
        check_capacity(db, project.id, result["duration_s"])
...
def generate(id: str, body: GenerationCreate, db: DB, idempotency_key: Key = None):
    ...
    with resource_slot("generation-project-" + project_id, 1):
        ...
        try:
            ...
            check_capacity(db, project.id, body.duration_s)
        except GenerationError as error:
