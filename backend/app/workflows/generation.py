...
from app.services.generation import generation_context, check_capacity, reserved_generation_duration
    ...
    with resource_slot("video-generation", settings.video_generation_concurrency):
        ...
        if not data.get("provider_task_id"):
            with SessionLocal() as db:
                ...
                check_capacity(db, project_id, reserved_generation_duration(data["duration_s"]), exclude_job_id=job.id)
