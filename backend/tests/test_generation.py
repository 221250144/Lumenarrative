...
def test_options_format_full_references_before_truncating_without_mutating_source(client, context):
    shot_id = uid()
    prefix = "依据输入首帧生成一段自然真实的 Vlog 补充镜头。\n镜头目的：\n补全建议："
    with SessionLocal() as db:
        task = db.get(CompletionTask, context.task_id)
        evidence = db.get(Evidence, task.data["reference_evidence_ids"][0])
        run = db.get(AnalysisRun, context.analysis_id)
        task.data = {
            **task.data, "requirement_description": "",
            "instruction": "铺" * (2000 - len(prefix) - 18) + " " + shot_id,
            "acceptance_checks": [f"shot_id '{shot_id}' remains visible"],
        }
        evidence.data = {**evidence.data, "action": "参" * 285 + " " + evidence.id}
        run.data = {**run.data, "shots": [{"id": shot_id, "start_s": 1.5, "end_s": 3.5}]}
        original_task, original_evidence, original_run = deepcopy(task.data), deepcopy(evidence.data), deepcopy(run.data)
        evidence_id = evidence.id
        db.commit()
    options = client.get(context.url.removesuffix("generations") + "generation-options").json()
    assert options["available"]
    assert len(options["prompt"]) <= 2000
    assert "1.5–3.5 秒" in options["prompt"]
    assert shot_id[:8] not in options["prompt"]
    reference = options["reference_frames"][0]
    assert len(reference["caption"]) <= 300 and "2–4 秒" in reference["caption"]
    assert evidence_id[:8] not in reference["caption"]
    assert reference["asset_id"] == context.asset_id and reference["time_s"] == 2
    assert reference["url"] == f"/api/v1/assets/{context.asset_id}/frame?at=2"
    with SessionLocal() as db:
        assert db.get(CompletionTask, context.task_id).data == original_task
        assert db.get(Evidence, evidence_id).data == original_evidence
        assert db.get(AnalysisRun, context.analysis_id).data == original_run
...
def test_requested_five_seconds_needs_margin_before_any_billable_submission(client, context, monkeypatch):
    calls = fake_generation(monkeypatch)
    with SessionLocal() as db:
        db.get(Asset, context.asset_id).duration_s = 595
        db.commit()
    options = client.get(context.url.removesuffix("generations") + "generation-options").json()
    assert not options["available"] and "时长" in options["reason"]
    assert post(client, context).status_code == 409
    assert client.get(context.url).json() == []
    assert calls["submit"] == 0


def test_reserved_margin_protects_pending_capacity_but_is_not_added_again_on_store(client, context, monkeypatch):
    calls = fake_generation(monkeypatch)
    monkeypatch.setattr(workflow, "probe", lambda _: {"duration_s": 5.125, "width": 1280, "height": 720, "has_audio": True, "source_start_time": 0})
    with SessionLocal() as db:
        db.get(Asset, context.asset_id).duration_s = 594.5
        db.commit()
    options = client.get(context.url.removesuffix("generations") + "generation-options").json()
    assert options["available"]
    response = post(client, context)
    ...
    with SessionLocal() as db:
        # The queued 5s request reserves the last 5.5 seconds, including its tail.
        with pytest.raises(GenerationError, match="时长"):
            check_capacity(db, context.project_id, 0.1)
    execute(job_id)
    job = client.get(f"/api/v1/jobs/{job_id}").json()
    assert job["status"] == "succeeded"
    with SessionLocal() as db:
        assert db.get(Asset, job["asset_id"]).duration_s == 5.125
        total = sum(asset.duration_s for asset in db.scalars(select(Asset).where(Asset.project_id == context.project_id)))
        assert total == 599.625
    assert calls["submit"] == 1


def test_worker_rechecks_the_generation_duration_margin_before_post(client, context, monkeypatch):
    calls = fake_generation(monkeypatch)
    job_id = post(client, context).json()["job_id"]
    with SessionLocal() as db:
        db.get(Asset, context.asset_id).duration_s = 595
